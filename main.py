...
async def start_cp_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only CP batch upload start - SIMPLIFIED flow"""
    if update.effective_user.id not in ADMIN_USER_IDS:
        await update.message.reply_text("❌ Access Denied!")
        return ConversationHandler.END
    
    if CP_CHANNEL_ID == 0:
        await update.message.reply_text("❌ CP_CHANNEL_ID not set in env!")
        return ConversationHandler.END
    ...
    await update.message.reply_text(
        "🔞 <b>CP UPLOAD MODE</b>\n\n"
        "📋 <b>EXACT FLOW:</b>\n"
        "1️⃣ Video bhejo → CP_CHANNEL_ID mein save\n"
        "2️⃣ Image bhejo → YEH FREE CHANNEL POSTER\n"
        "3️⃣ Amount type karo\n\n"
        "❌ Cancel: /cancel",
        parse_mode='HTML'
    )
...
async def get_cp_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save ONLY video to CP channel, NO thumbnail extraction"""
    msg = update.message
    ...
    if not msg.video and not msg.document:
        await msg.reply_text("❌ Video file bhejo!")
        return CP_WAIT_VIDEO
    
    # Title extract
    raw_caption = msg.caption if msg.caption else ""
    title = clean_title(raw_caption) or f"CP #{int(datetime.now().timestamp())}"
    
    # 🔥 ONLY SAVE VIDEO TO CP_CHANNEL - NO THUMBNAIL
    status = await msg.reply_text("⏳ Saving video to CP channel...")
    try:
        copied = await context.bot.copy_message(
        ...
        logger.info(f"✅ CP Video saved: {title}")
    except Exception as e:
        await status.edit_text(f"❌ CP channel upload failed: {e}")
        return CP_WAIT_VIDEO
    
    # Save video data (NO poster yet)
    context.user_data['cp_title'] = title
    context.user_data['cp_video_url'] = video_file_url
    
    await status.edit_text(
        f"✅ <b>Video Saved to CP Channel!</b>\n\n"
        f"📝 Title: <b>{html_escape(title)}</b>\n"
        f"🔗 Video URL: <code>{video_file_url}</code>\n\n"
        f"🖼️ <b>NEXT: Poster IMAGE bhejo</b>\n"
        f"📌 Yeh image FREE channel mein dikhega\n"
        f"👤 User ko bhi yahi milega!\n\n"
        f"❌ Cancel: /cancel",
        parse_mode='HTML'
    )
...
async def get_cp_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Save EXACT image as poster - user will get this"""
    msg = update.message
    ...
    if not msg.photo:
        await msg.reply_text(
            "❌ <b>PHOTO CHAHIE!</b>\n\n"
            "🖼️ Sirf IMAGE bhejo (poster ke liye)\n"
            "📌 Yeh user ko milega exactly!"
        )
        return CP_WAIT_IMAGE
    
    # 🔥 SAVE THIS EXACT IMAGE FILE_ID
    poster_file_id = msg.photo[-1].file_id  # Highest quality
    
    # Test karo ki file_id valid hai
    try:
        ...
        logger.info("✅ Poster file_id validated")
    except Exception as e:
...
async def get_cp_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all([title, video_url, poster_id]):
        await msg.reply_text("❌ Missing data! /cp se restart karo.")
        context.user_data.clear()
        return ConversationHandler.END
    ...
    try:
        ...
        conn.commit()
        cur.close()
        logger.info(f"✅ CP #{cp_id} created: {title}")
    except Exception as e:
        logger.error(f"❌ CP DB error: {e}")
    ...
    await msg.reply_text(
        f"🎉 <b>CP VIDEO READY!</b>\n\n"
        f"🆔 <b>ID:</b> <code>{cp_id}</code>\n"
        f"📝 <b>Title:</b> {html_escape(title)}\n"
        f"🖼️ <b>Poster:</b> ✅ Saved (user ko yahi milega)\n"
        f"📹 <b>Video:</b> CP_CHANNEL_ID mein\n"
        f"💰 <b>Price:</b> ₹{amount}\n\n"
        f"🔗 <b>Link:</b> <code>/start cp_{cp_id}</code>\n\n"
        f"👤 User 'Watch CP' dabayega → YEH POSTER + payment\n"
        f"✅ Payment → YEH POSTER + video forward",
        parse_mode='HTML'
    )
...
async def show_cp_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show CP list with EXACT poster images"""
    msg = update.message
    chat_id = msg.chat_id
    user = update.effective_user

    # Database se CP videos (with poster_file_id)
    conn = None
    cp_videos = []
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute(
            "SELECT cp_id, title, poster_file_id, price "
            "FROM cp_videos WHERE poster_file_id IS NOT NULL "
            "ORDER BY created_at DESC LIMIT 20"
        )
        cp_videos = cur.fetchall()
        cur.close()
    except Exception as e:
        logger.error(f"CP list error: {e}")
        return
    finally:
        if conn:
            db_pool.putconn(conn)
    ...
    if not cp_videos:
        await msg.reply_text("❌ No CP videos available!")
        return

    await msg.delete()  # Start message delete

    # Har video ka EXACT poster bhejo
    for cp_id, title, poster_id, price in cp_videos:
        caption = (
            f"🔞 <b>{html_escape(generate_display_title(title))}</b>\n\n"
            f"💰 <b>Price: ₹{price}</b>\n\n"
            f"👤 Buy → Payment → Yeh poster + video milega!"
        )

        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"💸 Buy ₹{price}", callback_data=f"cp_buy_{cp_id}")]
        ])

        try:
            # ✅ EXACT POSTER IMAGE bhejo
            sent_msg = await context.bot.send_photo(
                chat_id=chat_id,
                photo=poster_id,  # Yeh wahi image jo admin ne bheji thi
                caption=caption,
                parse_mode='HTML',
            ...
            logger.error(f"❌ CP poster send failed {cp_id}: {e}")
